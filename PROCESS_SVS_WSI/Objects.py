...
class HistoImage(tf.keras.utils.Sequence):
    def __init__(self, filename, batch_size=16, target_size=(512, 512), target_magnification=20, overlap_rate=(0.99,  0.99), preprocessing_function=None, verbose=0) -> None:
        ...
        self.ImageMagnification = int(
            self.image.properties['openslide.objective-power'])
    ...
    def GetThumbnails(self):
        # get_thumbnail((512,512))#get_thumbnail((2048,2048))#
        self.ThumbnailImage = self.image.read_region((0, 0), len(
            self.image.level_dimensions)-1, self.image.level_dimensions[-1])
        w, h = self.ThumbnailImage.size
        self.ThumbnailMask = cv2.resize(
    ...
    def __getitem__(self, idx):
        ...
        X_batch = [np.array(self.image.read_region(x_b[:2], 0, x_b[2:]).resize(
            self.target_size).convert("RGB")) for x_b in X_batch_tmp]
...
if UnitTest:
    img = HistoImage(
        "../../Diagnostic_image/ef2e8fd8-33e1-40b8-9daa-4e91c41b3f5c/TCGA-X4-A8KS-01Z-00-DX4.34C5D87E-7598-49A7-B937-6CD2D7D64811.svs", verbose=1299)
    print(len(img))
    for batch in img:
