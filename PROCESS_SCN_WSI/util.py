...
def GetImagePatchCoordinationFromMaskFile(filename,
    ...
    mask = GetMaskWithReference(openslide.OpenSlide(
        MaskFilename), threshold=threshold, strategy=strategy)
    ...
    for region in regions_bb:
        ...
        Pt_bottom_right = ((max_col/w_tiny)*w*factor,
                           (max_row/h_tiny)*h*factor)
...
class OpenSlide(tf.keras.utils.Sequence):
    ...
    def GetImage(self, filename, region):
        ...
        # location=region, level=0, size=(size,size))
        img = image.crop(region[0], region[1], size, size)
